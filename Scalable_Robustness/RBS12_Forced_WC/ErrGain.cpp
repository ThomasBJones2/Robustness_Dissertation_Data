...
#define ERR_INPUT_NUM 1			//This the number of inputs and error patterns which will be executed to check for criticallity
...
//			cerr << "J: " << array[j] << " I: " << 
//				array[i] << " CJ: " << Counts[i] 
//				<< " PF: " << Pattern_Func(ErrPattern,count) << "\n";
			count ++;	
			}
		}
	}
...
//		cerr << " " << arg2[i];	
//	cerr << "\n";
//	cerr << "Max Pos Err: " << FindPositionError(arg2, Length)<< "\n";
...
//	  	CreateInput(arg, Length); 
		...
		cerr << "The unsorted List: ";	
		for(int i = 0; i < Length; i++)
			cerr << arg[i] << " ";
		cerr << "\n";

	  	GetCount = Merge_Sort(arg, Length, ErrPattern);
		cerr << "The sorted List: ";	
		for(int i = 0; i < Length; i++)
			cerr << arg[i] << " ";
		cerr << "\n";
		...
		for(int i = 0; i < GetCount; i ++){
			cerr << Pattern_Func(ErrPattern, i) << " ";
		}
		cerr << "\n";
...
//	  	CreateErrorPattern(ErrPattern, RATE, Length, MAX_ERR_PATTERN_LENGTH);
		Reverse_List(arg, Length);
//	  	CreateInput(arg, Length); 
//		cerr << "CCS The input List: ";	
//		for(int i = 0; i < Length; i++)
//			cerr << arg[i] << " ";
//		cerr << "\n";
	  	GetCount = Comp_Count_Sort(arg, Length, ErrPattern);
//		cerr << "CCS The sorted List: ";	
//		for(int i = 0; i < Length; i++)
//			cerr << arg[i] << " ";
//		cerr << "\n";
